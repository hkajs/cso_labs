...
list_remove(struct list_node **headp, int value)
{
	...
	assert(*headp != NULL);
	// TODO: Your code here.
	if ((*headp) -> value == value){
		struct list_node *temp = *headp;
		*headp = (*headp) -> next;
		free_node(temp);
		return 1;
	}
	struct list_node *predp = alloc_node();
	struct list_node **predpp = &(predp);
	struct list_node *node = list_find(*headp, value, predpp);
	if (node != NULL){
		(*predpp) -> next = node -> next;
		free_node(node);
		return 1;
	}
	return 0;
